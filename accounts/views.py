...
def home(request):
    ...
    if user.is_patient==True:
        try:
            ...
            readings.heart_beat = randint(55, 120)
            readings.SpO2 = randint(85, 100)
            readings.body_temperature = randint(30, 45)
            readings.blood_pressure_Diastolic = randint(70, 100)
            readings.blood_pressure_Systolic = randint(110, 150)
        ...
        try:
            ...
            try:
                ...
                return HttpResponseRedirect(reverse('accounts:addressInfo'))
...
def patientDetail(request, id):
    user = CustomUser.objects.get(pk=request.session['_auth_user_id'])
    doctor = Doctor.objects.get(doctor_id_id=user)
    mypatient = Patient.objects.get(pk=id)
    dob = mypatient.date_of_birth
    mypatientAge = calculate_age(dob)

    readings = Readings.objects.filter(patient_id_id=mypatient)[:10:-1]


    return render(request, 'accounts/patientDetail.html', {'Doctor':doctor, 'mypatient':mypatient, 'mypatientAge':mypatientAge})
...
def view_products(request):
    if request.user.is_authenticated:
        ...
        if request.method=='POST':
            patient = Patient.objects.get(patient_id_id=user)
            if patient.has_device==True:
                return render(request, 'accounts/ProductFamily.html',
                              {'Authenticated': "Authenticated", 'alreadyHaveaDevice': "alreadyHaveaDevice"})
                ...
                return render(request, 'accounts/ProductFamily.html', {'Authenticated':"Authenticated",'Success': "You have succesfully bought this device"
                                                                                                  ",with this you can measure your health parameters and get medication from your dedicated doctor."})

        return render(request, 'accounts/ProductFamily.html',{'Authenticated':"Authenticated"})
    elif request.user.is_anonymous:
        return render(request, 'accounts/ProductFamily.html', {'Not_Authenticated': "Not_Authenticated"})



@login_required()
def get_Patient_Readings_data_as_json_fromDB(request, id):
